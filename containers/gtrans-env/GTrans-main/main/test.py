...
def add_test_args(parser):
    ...
    files.add_argument('--dev_gnn', nargs='+', type=str, required=True,
                       help='Preprocessed dev gnn file')
...
def set_defaults(args):
    ...
    for i in range(num_dataset):
        ...
        dev_gnn = os.path.join(data_dir, args.dev_gnn[i])
        ...
        args.dev_gnn_files.append(dev_gnn)
...
def prepare_batch(batch, model):
    ...
    nodes = batch['nodes_feature']
    adjacency_matrix = batch['adjacency_matrix']
    backbone_sequence = batch['backbone_sequence']
    lengths_backbone = batch['lengths_backbone']
    max_lengths = max(lengths_backbone).item()
    lengths_node = batch['lengths_node']
    ...
    if args.cuda:
        ...
        if adjacency_matrix is not None:
            adjacency_matrix = adjacency_matrix.cuda(non_blocking=True)
        if backbone_sequence is not None:
            backbone_sequence = backbone_sequence.cuda(non_blocking=True)
        if lengths_backbone is not None:
            lengths_backbone = lengths_backbone.cuda(non_blocking=True)
        if lengths_node is not None:
            lengths_node = lengths_node.cuda(non_blocking=True)
    ...
    batch_inputs['nodes'] = nodes
    batch_inputs['adjacency_matrix'] = adjacency_matrix
    batch_inputs['backbone_sequence'] = backbone_sequence
    batch_inputs['max_lengths'] = max_lengths
    batch_inputs['lengths_backbone'] = lengths_backbone
    batch_inputs['lengths_node'] = lengths_node
...
def main(args):
    ...
    dev_exs = []
    for dev_src, dev_src_tag, dev_tgt, dev_gnn, dataset_name in \
            zip(args.dev_src_files, args.dev_src_tag_files,
                args.dev_tgt_files, args.dev_gnn_files, args.dataset_name):
        dev_files = dict()
        dev_files['src'] = dev_src
        dev_files['src_tag'] = dev_src_tag
        dev_files['tgt'] = dev_tgt
        dev_files['gnn'] = dev_gnn
